...
import { Loader2, Car, PlusCircle, Upload } from 'lucide-react';
...
export default function TransportExpensesPage() {
  ...
  const transportExpensesQuery = useMemoFirebase(() => {
    if (!user) return null;
    return query(
  ...
  const { data: transportExpenses, isLoading: isExpensesLoading } = useCollection<Transaction>(transportExpensesQuery);
  ...
  return (
    <>
      ...
      />
       ...
       <PageHeader
        title="Transporte"
        description="Controle seus gastos com combustível, transporte público, aplicativos e manutenção do veículo."
      >
      ...
      {transportExpenses && transportExpenses.length > 0 ? (
        ...
        <Card>
            <CardContent className="flex flex-col items-center justify-center py-12 px-4 text-center">
                ...
                <h3 className="text-lg font-semibold mb-2">Nenhuma despesa de transporte cadastrada</h3>
                <p className="text-sm text-muted-foreground max-w-sm mb-6">
                    Registre aqui seus gastos com transporte para ter uma visão completa de sua mobilidade.
                </p>
                <Button onClick={() => handleOpenSheet()} disabled={!user}>
                    <PlusCircle className="mr-2 h-4 w-4" />
