...
import { Loader2, Clapperboard, PlusCircle, Upload } from 'lucide-react';
...
import { leisureSubcategories } from '@/lib/types';

export default function LeisureExpensesPage() {
  ...
  const leisureExpensesQuery = useMemoFirebase(() => {
    if (!user) return null;
    return query(
      collection(firestore, `users/${user.uid}/expenses`),
      where('category', '==', 'Lazer'),
      orderBy('date', 'desc')
    );
  }, [user, firestore]);

  const { data: leisureExpenses, isLoading: isExpensesLoading } = useCollection<Transaction>(leisureExpensesQuery);
  ...
  return (
    <>
      ...
      />
       ...
       <PageHeader
        title="Lazer"
        description="Gerencie seus gastos com restaurantes, viagens, hobbies e outras atividades de lazer."
      >
      ...
      {leisureExpenses && leisureExpenses.length > 0 ? (
        <DataTable
            columns={columns}
        ...
        <Card>
            <CardContent className="flex flex-col items-center justify-center py-12 px-4 text-center">
                ...
                <h3 className="text-lg font-semibold mb-2">Nenhuma despesa de lazer cadastrada</h3>
                <p className="text-sm text-muted-foreground max-w-sm mb-6">
                    Registre aqui seus gastos com cinema, restaurantes, viagens e hobbies para ter uma visão clara de como você se diverte.
                </p>
                <Button onClick={() => handleOpenSheet()} disabled={!user}>
                    <PlusCircle className="mr-2 h-4 w-4" />
